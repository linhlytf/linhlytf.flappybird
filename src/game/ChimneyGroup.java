...
public class ChimneyGroup {
    ...
    private int topChimneyY = -280;
    private int botChimneyY = 280;
    ...
    public ChimneyGroup() {
        try {
            chimneyImage = ImageIO.read(new File("src/game/chimney.png"));
            chimneyImage2 = ImageIO.read(new File("src/game/chimney2.png"));
        } catch (IOException ex) {}
        chimneys = new QueueList<Chimney>();
        Chimney cn;
        for (int i = 0; i < SIZE / 2; i++) {
            int deltaY = getRandomY();
            cn = new Chimney(830 + i * 300, botChimneyY + deltaY, 74, 400);
            chimneys.push(cn);
